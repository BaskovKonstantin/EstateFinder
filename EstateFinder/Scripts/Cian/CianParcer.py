from Param import PARAM_SPECS
import urllib.parse
import time
from EstateFinder.Scripts.Classes.RealEstate import RealEstate
import re
import json
...
def extract_cian_ad_links(html_content: str) -> list[str]:
    """
    Извлекает из переданного HTML-контента все уникальные ссылки на объявления о продаже квартир на CIAN.

    Параметры:
        html_content (str): HTML-строка страницы.

    Возвращает:
        list[str]: Список уникальных URL объявлений.
    """
    ...
    for a in soup.find_all('a', href=True):
        ...
        if re.match(r"^https?://www\.cian\.ru/sale/flat/\d+/?$", href):
            links.add(href)
        # Относительный URL объявления
        elif re.match(r"^/sale/flat/\d+/?$", href):
            links.add(requests.compat.urljoin('https://www.cian.ru', href))
    return sorted(links)
